...
func DiemDanhNhom(body map[string]interface{}, payload map[string]interface{}) string {
	DATA := body["DATA"].(map[string]interface{})
	//get team name list from DATA
	team_name_list := DATA["team_name_list"]
	JOB_NAME := payload["JOB_NAME"].(string)
	SUBDEPTNAME := payload["SUBDEPTNAME"].(string)
	ATT_GROUP_CODE := payload["ATT_GROUP_CODE"].(float64)
	condition := ""
	switch team_name_list {
	case "0":
	...
	newJson := map[string]interface{}{
		"tk_status": "NG",
		"message":   "Bạn không phải leader, sub leader, dept staff, admin",
	}
	resultJson, err := json.Marshal(newJson)
	if err != nil {
		log.Fatal("Error marshalling JSON:", err.Error())
